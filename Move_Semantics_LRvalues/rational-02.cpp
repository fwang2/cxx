...
public:
    ...
    // NOTE: to support move semantic, we must define move ctor
    // this is the signature, noexcept is must
...
// NOTE: this is the definition of move ctor
// we use std::move() to move individual member
// finally, we reset the rhs.

Rational::Rational(Rational && rhs) noexcept {
    message("move ctor");
    _n = std::move(rhs._n);
    _d = std::move(rhs._d);
    rhs.reset();  // set reset definition; the goal is to leave source object in a sane state
}
...
int main() {
    ...
    // NOTE: without move ctor, copy ctor will be called
    // Now that we have move ctor, we will observe move ctor is called.
    //
    // std::move() cast the object c as a Rvalue
    // you can change
    // std::move(c) 
    // as
    // (Rational &&)(c)

...
void gettoken(void) /* read next token into "this" */
{
    ...
    if (isalnum(*p)) {
        /* it starts with a A-Z, 0-9 read in identifier */
...
/* the piece of code underneath all parsing */
...
int main(int argc, char* argv[]) 
{
    ...
    char *example1 = "void (*signal(int sig, void (*func)(int))) (int);";
    char *example2 = "char *(*c[10])(int **p);";
    
    if (argc != 2) {
        printf("You can try examples such as: \n");
        printf("\t %s\n", example1);
        printf("or\n");
        printf("\t %s\n", example2);
        printf("\n");
